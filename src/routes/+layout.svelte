...
>
	<!-- ========== LEFT SIDEBAR (bisa dibuka di semua halaman) ========== -->
	...
	>
		...
		<div class="flex min-h-screen min-w-0 flex-1 flex-col">
			...
			>
				<div class="flex h-14 w-full items-center justify-between gap-2 px-3 sm:h-16 sm:gap-3 sm:px-5">
					...
					<div class="relative flex shrink-0 items-center gap-0.5">
						...
						<!-- History toggle (desktop, non-reader only) -->
						{#if !isReaderPage}
							<button
			...
			<!-- History Widget Mobile (non-reader only) -->
			{#if !isReaderPage}
		...
		<!-- Right: History Widget Desktop (non-reader only) -->
		{#if !isReaderPage}
			<div
