<script lang="ts">
	...
	function typeBadgeClass(type?: string) {
		...
		switch (t) {
			case 'doujinshi': return 'bg-[#8b1e42]';
			case 'artistcg':
			case 'gamecg': return 'bg-[#009688]';
			case 'imageset': return 'bg-[#616161]';
			case 'anime': return 'bg-[#7b1fa2]';
			case 'western': return 'bg-[#5d4037]';
			case 'non-h': return 'bg-[#455a64]';
			case 'manhwa': return 'bg-[#1976D2]';
			case 'manhua': return 'bg-[#2E7D32]';
			default: return 'bg-[#c91714]';
		}
	}
</script>
...
<div class="mx-auto w-full max-w-none px-2 py-3 sm:px-3 sm:py-4 lg:px-4">
	...
	<div class="mb-4 flex flex-wrap items-center gap-2 sm:gap-3">
		...
		<select
			...
			class="rounded-lg border px-3 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-violet-500 transition-colors {isDarkMode
		...
		<!-- Select Language (Hanya muncul jika cocok/berguna, seperti Hitomi) -->
{#if ['hitomi', 'nhentai'].includes(currentSource)}
	<select
...
{/if}
		...
		<!-- Input Search Form -->
		<form class="flex min-w-0 max-w-md flex-1 gap-2" onsubmit={handleSearch}>
			<div class="relative min-w-0 flex-1">
				...
				<input
					...
					class="w-full rounded-full border py-2 pr-4 pl-9 text-sm focus:outline-none focus:ring-1 focus:ring-violet-500 transition-colors {isDarkMode
			...
			<button
				type="submit"
				disabled={loading}
				class="shrink-0 rounded-full bg-violet-600 px-4 py-2 text-sm font-medium text-white transition hover:bg-violet-500 disabled:opacity-50"
			>
				{#if loading}
	...
	{#if searchQuery || selectedLang !== 'all' || selectedType !== 'all'}
		<p class="mb-3 text-sm {isDarkMode ? 'text-zinc-400' : 'text-zinc-600'}">
			Filtering active: 
			{#if searchQuery}<span class="font-semibold text-violet-400">"{searchQuery}"</span> {/if}
			{#if selectedLang !== 'all'}<span class="ml-2 rounded bg-violet-500/20 px-2 py-0.5 text-xs text-violet-300">Lang: {selectedLang}</span>{/if}
			{#if selectedType !== 'all'}<span class="ml-2 rounded bg-violet-500/20 px-2 py-0.5 text-xs text-violet-300">Type: {selectedType}</span>{/if}
			<a href="/?source={currentSource}" class="ml-3 text-violet-500 hover:underline">Clear all</a>
		</p>
	{/if}
				...
				<a href="/manga/{manga.sourceId}{manga.id}" class="group block">
					<div class="relative overflow-hidden rounded-md bg-zinc-900 ring-1 ring-black/5 dark:ring-white/5">
						<div class="relative aspect-[2/3] w-full overflow-hidden">
							...
							<span class="absolute bottom-[28px] left-1 z-20 rounded px-1 py-0.5 text-[8px] font-bold uppercase text-white sm:bottom-7 sm:text-[9px] {typeBadgeClass(manga.type)}">
								{manga.type || 'manga'}
							</span>

							<div class="absolute inset-x-0 bottom-0 z-10 bg-gradient-to-t from-black/95 via-black/55 to-transparent px-1 pb-1.5 pt-6">
								<h3 class="line-clamp-2 text-center text-[10px] font-semibold leading-tight text-white drop-shadow-md sm:text-[11px]">
									{manga.title}
								</h3>
							</div>
						</div>
