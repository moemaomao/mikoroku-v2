...
/**
 * NeloManga.net adapter (MangaBox / Manganelo family) (HTML + chapters JSON API)
 *
 * Domain   : https://www.nelomanga.net
 ...
 */
export class NeloMangaSource extends BaseSource {
	id = 'nelomanga';
	name = 'NeloManga';
	baseUrl = 'https://www.nelomanga.net';
	...
	protected async fetchHtml(path: string): Promise<string> {
		...
		if (!res.ok) {
			throw new Error(`NeloManga HTTP ${res.status} → ${url}`);
		}
	...
	private async fetchApiJson<T = any>(path: string): Promise<T> {
		...
		if (!res.ok) {
			throw new Error(`NeloManga API ${res.status} → ${url}`);
		}
	...
	async getLatestManga(
		...
		try {
			...
			console.log(`[nelomanga] latest page=${p} → ${out.length} items`);
			return out;
		} catch (e) {
			console.error('[nelomanga] getLatestManga', e);
			return [];
		}
	}
	...
	async searchManga(
		...
		try {
			...
			console.log(`[nelomanga] search "${q}" → ${list.length} items`);
			return list;
		} catch (e) {
			console.error('[nelomanga] searchManga', e);
			return [];
		}
	}
	...
	async getMangaDetails(
		...
		if (!slug) {
			throw new Error(`NeloManga: invalid mangaId ${mangaId}`);
		}
		...
		try {
			...
			if (cdnMatch && imgMatch) {
				try {
					...
					console.error('[nelomanga] parse cdns/chapterImages', e);
				}
			}
			...
			console.log(
				`[nelomanga] getChapterPages ${slug}/${chapterSlug} → ${pages.length} pages`
			);
			return pages;
		} catch (e) {
			console.error('[nelomanga] getChapterPages', e);
			return [];
		}
	}
}
